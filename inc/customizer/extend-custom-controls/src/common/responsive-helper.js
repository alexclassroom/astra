...
export function astraGetResponsiveNumberJs ( control ) {
    'use strict';

    let device = jQuery('.wp-full-overlay-footer .devices button.active').attr('data-device')

    jQuery( '.customize-control-ast-number .ast-responsive-container' ).removeClass( 'active' );

    jQuery( '.customize-control-ast-number .ast-responsive-container.' + device ).addClass( 'active' );

    jQuery( '.customize-control-ast-number .ast-responsive-btns li' ).removeClass( 'active' );

    jQuery( '.customize-control-ast-number .ast-responsive-btns li.' + device ).addClass( 'active' );

    jQuery('.wp-full-overlay-footer .devices button').on('click', function() {

        let device = jQuery(this).attr('data-device');

        jQuery( '.customize-control .ast-responsive-container, .customize-control .ast-responsive-btns  > li' ).removeClass( 'active' );
        jQuery( '.customize-control .ast-responsive-container.' + device + ', .customize-control .ast-responsive-btns  > li.' + device ).addClass( 'active' );
    });

    control.container.find( '.ast-responsive-btns button svg' ).on( 'click', function( event ) {
        event.preventDefault();
        let device = jQuery(this).parent('button').attr('data-device');
        if( 'desktop' == device ) {
            device = 'tablet';
